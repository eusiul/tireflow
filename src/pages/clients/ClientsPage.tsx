...
export function ClientsPage() {
  ...
  const openEdit = (c: Client) => {
    ...
    setForm({
      ...
      document: c.document ?? '', vehiclePlates: c.vehiclePlates.join(', '), notes: c.notes ?? '',
    })
    setFormError('')
    setFormOpen(true)
  }
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setFormError(e?.message || 'Erro ao salvar cliente.')
    } finally {
  ...
  return (
    <div className="p-6 max-w-7xl mx-auto">
      ...
      >
        <div>
          ...
          <p className="text-sm text-zinc-500 mt-0.5">
            {loading ? 'Carregando...' : `${clientList.length} clientes cadastrados`}
          </p>
        </div>
        <Button variant="primary" size="sm" icon={<Plus size={14} />} onClick={openCreate}>Novo Cliente</Button>
      </motion.div>
      ...
      >
        {[
          { label: 'Total de clientes', value: clientList.length, icon: <Users size={15} />, format: 'number' as const },
          { label: 'Receita total', value: totalRevenue, icon: <TrendingUp size={15} />, format: 'currency' as const },
          { label: 'Ticket médio', value: avgSpent, icon: <Star size={15} />, format: 'currency' as const },
        ].map((s) => (
      ...
      <div className="flex gap-4">
            ...
            <div className="space-y-2">
              {clientList.map((client, i) => (
                <motion.button
                  ...
                  className={cn(
                    'w-full text-left surface-card p-4 hover:border-surface-600 transition-all duration-150 flex items-center gap-4',
                ...
                >
                  ...
                  <div className="text-right shrink-0">
                    ...
                    <p className="text-xs text-zinc-500">{client.totalVisits} visitas</p>
                  </div>
        ...
        <motion.div
          initial={false}
          animate={{ width: selected ? 320 : 0, opacity: selected ? 1 : 0 }}
            ...
            <div className="surface-card p-4 sticky top-4 w-80">
              ...
              <div className="grid grid-cols-2 gap-2 mb-4">
                {[
                  ...
                  { label: 'Visitas', value: String(selected.totalVisits) },
                  { label: 'Última visita', value: selected.lastVisit ? formatRelativeTime(selected.lastVisit) : '—' },
                  { label: 'Veículos', value: String(selected.vehiclePlates.length) },
                ].map(({ label, value }) => (
              ...
              {selected.vehiclePlates.length > 0 && (
                <div className="mb-4">
                  <p className="text-xs text-zinc-500 mb-2 font-medium">Placas</p>
      ...
      >
        <form onSubmit={handleSave} className="space-y-4">
          ...
          <Input
            ...
            value={form.vehiclePlates}
            onChange={(e) => setForm((f) => ({ ...f, vehiclePlates: e.target.value }))}
          ...
          <div className="flex items-center gap-2 pt-1">
            <Button type="submit" variant="primary" loading={formLoading}>
              {formLoading ? 'Salvando...' : (editTarget ? 'Salvar alterações' : 'Criar cliente')}
            </Button>
